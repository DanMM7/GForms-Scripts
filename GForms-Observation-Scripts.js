...
setTimeout(function() { 
    ...
    if(color==1) 
        $('#FORM_WD_Inspection_TarpLevel').val('High - Stop and Fix (Red)') ; 
    else if(color==2) 
        $('#FORM_WD_Inspection_TarpLevel').val('Medium - Fix within 24 Hours (Orange)') ; 
    else if(color==3) 
        $('#FORM_WD_Inspection_TarpLevel').val('Low - Fix Within Schedule (Yellow)') ; 
    else if(color==4) 
        $('#FORM_WD_Inspection_TarpLevel').val('Normal Operating Conditions (Green)');
},1500); 
...
// Script for tarp
if('{controlvalue}'.includes('Normal Operating Conditions')) { 
    $('{this}').css('background-color', 'green').css('color','black'); 
} 
else if('{controlvalue}'.includes('High - Stop and Fix')) { 
    $('{this}').css('background-color', 'red').css('color','white'); 
} 
else if('{controlvalue}'.includes('Medium - Fix within 24 Hours')) { 
    $('{this}').css('background-color', 'orange').css('color','white'); 
} 
else if('{controlvalue}'.includes('Low - Fix Within Schedule')) { 
    $('{this}').css('background-color', 'yellow').css('color','black'); 
} 
else { 
...
else if('{controlvalue}'.includes('High - Stop and Fix')) { $('{this}').css('background-color', 'red').css('color','white'); } else if('{controlvalue}'.includes('Medium - Fix within 24 Hours')) { $('{this}').css('background-color', 'orange').css('color','white'); } else if('{controlvalue}'.includes('Low - Fix Within Schedule')) { $('{this}').css('background-color', 'yellow').css('color','black'); } else if('{controlvalue}'.includes('[NULL]')) { $('{this}').css('background-color', 'white').css('color','Grey'); }; setTimeout(function() { var color = 4; var selector = '[container=childResults] td:first'; var iStart = 15; for(var i =0; i<9; i++){ if($('[fieldid='+(iStart+i)+']' + selector).text().includes('(Red)')) color=1; else if($('[fieldid='+(iStart+i)+']' + selector).text().includes('(Orange)') && color>1) color=2; else if($('[fieldid='+(iStart+i)+']' + selector).text().includes('(Yellow)') && color>2) color=3; else if($('[fieldid='+(iStart+i)+']' + selector).text().includes('(Green)') && color>3) color=4; } if(color==1) $('#FORM_Geotechnical_Inspection_TarpLevel').val('High - Stop and Fix (Red)') ; else if(color==2) $('#FORM_Geotechnical_Inspection_TarpLevel').val('Medium - Fix within 24 Hours (Orange)') ; else if(color==3) $('#FORM_Geotechnical_Inspection_TarpLevel').val('Low - Fix Within Schedule (Yellow)') ; else if(color==4) $('#FORM_Geotechnical_Inspection_TarpLevel').val('Normal Operating Conditions (Green)'); },200); setTimeout(function() { $('#FORM_Geotechnical_Inspection_TarpLevel').change() },201);
...
//Hide child fields
ABSP_CREWDECLAR_COMP=No
ABSP_DemarkTopBot_Comp=No
ABSP_Declaration_Comp=No
ABSP_PsPbInstruct_Comp=No
